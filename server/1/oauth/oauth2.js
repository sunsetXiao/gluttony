...
OAuth2.prototype.getAuthorizeUrl = function(scope) {
  ...
  return util.addUrlParam(authorizeUrl, query);
};

/*
code[, extra], callback
*/
OAuth2.prototype.getAccessToken = function(code, extra, callback) {
 ...
 */
OAuth2.prototype.request = function (options, params, accessToken, multi, callback) {
  ...
  req = https.request(options, function (res) {
    ...
    res.on('end', function () {
      ...
      if (config.platform == "qq" && "grant_type" in params) {
        data = url.parse(config.base_uri + "?" + result, true).query;        
      } else {
      ...
      if (res.statusCode != 200 && res.statusCode != 301 && res.statusCode != 302) {
        var error = new Error();
        error.message = data.error_code + " " + data.error + ", when request = " + data.request;
...
OAuth2.prototype.getUniqueBoundary = function() {
...
OAuth2.prototype.getBody = function(params, multi) {
  if (!multi) {
